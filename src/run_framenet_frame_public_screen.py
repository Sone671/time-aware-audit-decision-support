#!/usr/bin/env python
"""Frozen public v2 screen for FrameNet frame-disambiguation judgments."""
...
from framenet_frame_public import add_leave_one_out_risk, load_public_records  # noqa: E402
...
OUTPUT = ROOT / "outputs" / "framenet_frame_public_screen"
PROTOCOL = ROOT / "V2_FRAMENET_FRAME_PUBLIC_PROTOCOL.md"
...
def summarize(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if not eligible:
        raise ValueError("no eligible FrameNet public records")
...
def run(_: argparse.Namespace) -> Path:
    ...
    structure_passed = bool(
        ...
        and risk_audit["unit_candidate_mapping_violation_count"] == 0
        and parse_audit["unit_candidate_mapping_violation_count"] == 0
        and parse_audit["unit_alignment_key_violation_count"] == 0
    )
    passed = bool(structure_passed and pooled["route"] == "risk_per_second")
    payload = {
        "screen": "framenet_frame_disambiguation_public_v2_screen_v1",
        ...
        "decision": {
            "license_gate": parse_audit["license"] == "CC-BY-SA-4.0",
            "cost_cv_gate": pooled["cost_cv"] >= 0.50,
            "minimum_opportunity_gate": pooled["public_opportunity"]["minimum_saving"]
            >= 0.40,
            "repeated_distinct_workers_gate": risk_audit["undersized_group_count"] == 0
            and risk_audit["duplicate_worker_group_count"] == 0,
            "candidate_mapping_gate": risk_audit[
                "unit_candidate_mapping_violation_count"
            ]
            == 0
            and parse_audit["unit_alignment_key_violation_count"] == 0,
            ...
            "truth_opening_blockers": (
                [
                    "independent FrameNet 1.7 source and exact key-only alignment",
                    "complete expert-frame candidate coverage",
                    "full executable pre-private method lock",
                ]
                if passed
                else ["public intervention gates"]
            ),
        },
    }
