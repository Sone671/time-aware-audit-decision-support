#!/usr/bin/env python
"""Frozen public v2 intervention screen for CrowdTruth medical RelEx."""
...
ARCHIVE = (
    ...
    / "crowdtruth_medical_relex"
    / "Medical-Relation-Extraction-585c1588.zip"
)
OUTPUT = ROOT / "outputs" / "crowdtruth_medical_public_screen"
...
def run(_: argparse.Namespace) -> Path:
    ...
    if not eligible:
        raise ValueError("no repeated CrowdTruth RelEx records")
    ...
    all_cells_repeated = risk_audit["undersized_group_count"] == 0
    passed = bool(route == "risk_per_second" and all_cells_repeated)
    payload = {
        "screen": "crowdtruth_medical_relex_public_v2_intervention_screen_v1",
        ...
        "decision": {
            ...
            "truth_opening_blockers": (
                ["license clarification"] if passed else ["public intervention gates"]
            ),
        },
    }
